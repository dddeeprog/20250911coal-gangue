...
function lineshowtrend(datashow,wave,wave_slec,name,save,flag,varargin)
    defaultcoe = 0;
    p = inputParser;
    ...
    parse(p,datashow,wave,wave_slec,name,save,flag,varargin{:});
    % 读取是否需要排序
    sortmode = 0;
    ...
    for j = 1:length(peak_positions)
        ...
        plot(line,'LineWidth',1.5,'color','b')
        text('string',txt,'Units','normalized','position',[0.65,0.1],"FontName",'times new roman','FontWeight','Bold')
        plotstyle('ptitle',[name{j},' ',num2str(wave_slec(j)),'nm'],'x','width','y','height')
