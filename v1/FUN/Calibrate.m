%{
20220718-HWH
定标函数以及相关参数指标计算Calibrate用于水质检测定标
...
可选参数：
yanslec:默认：不使用yanslec选择合适的区域 [14,14]：使用并展示
    'yanslec',[14,14] 传输给掩膜的形状
...
re = Calibrate(wave,dataset1,Cd,label,'yanslec',[14,14], ...
    'dataslec',1:6,'subbase',1,'fitmode',2,'save',1);

%}
function [re,mat_av,mat_std] = Calibrate(wave,dataset,wave_slec,label,varargin)
    defaultcoe = 0;
    p = inputParser;
    ...
    addOptional(p,'yanslec',defaultcoe,validScalarPosNum);
    ...
    if p.Results.yanslec ~= 0 
        yanslec_flag = 1;
        yansize = p.Results.yanslec;
    end
...
%         None;
    ...
    for i = 1:class_num
        ...
        if yanslec_flag == 0
%             None;
        elseif yanslec_flag == 1
            element = {'H'};
            [yandata,yanmo,~] = yanslec(wave,dataset_t,yansize(1),yansize(2),element);
            % 展示掩膜选区绘图
            figure(1);
            subplot(4,4,i);
            imagesc(yanmo);
