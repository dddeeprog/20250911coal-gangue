...
function [S,wave,msg] = csv2mat2(basepath)
...
function [S,wave] = makedataset(basepath,topli)
    try
        ...
        elseif length(topli)==2 %一级目录，二级文件
            % 读取波长文件
            ...
            set(h,'doublebuffer','on');
            for i = 1:len1
                    line = readmatrix([basepath,'/',topli{1}{i},'/',topli{2}{1}]);
                    line = line(:,2:end)';
                    dataset(len2*(i-1)+1:len2*i,:) = line;
                str=['正在计算类别',num2str(i),'中...'];
                waitbar(i/len1,h,str);
        ...
        elseif length(topli)==3%一级目录，二级子目录(激发的点数），三级文件
            ...
            len1 = length(topli{1});
            len2 = length(topli{2});
            len3 = size(readmatrix(wavepath),2) - 1;
            ...
            for num = 1:len2
                ...
                for i = 1:len1
                        line = readmatrix([basepath,'/',topli{1}{i},'/',topli{2}{num},'/',topli{3}{1}]);
                        line = line(:,2:end)';
                        dataset(len3*(i-1)+1:len3*i,:) = line;
